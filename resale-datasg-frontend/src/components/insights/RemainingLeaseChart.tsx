...
import type { RemainingLeasePriceResponse } from '../../api/types'
import { CURRENCY_COMPACT, niceAxisTicks, thinIndices } from './chartUtils'
import styles from './RemainingLeaseChart.module.css'
...
interface RemainingLeaseChartProps {
  data: RemainingLeasePriceResponse[]
  description?: string
}

export function RemainingLeaseChart({ data, description }: RemainingLeaseChartProps) {
  const wrapperRef = useRef<HTMLDivElement>(null)
  const [hoverIndex, setHoverIndex] = useState<number | null>(null)
  const [hoverPos, setHoverPos] = useState<{ x: number; y: number } | null>(null)

  const points = useMemo(() => [...data].sort((a, b) => a.remainingLeaseYears - b.remainingLeaseYears), [data])
  const labelIndices = useMemo(() => new Set(thinIndices(points.length, 12)), [points.length])

  if (points.length === 0) {
    return <p className={styles.empty}>No remaining lease data available.</p>
  }
  ...
  return (
    <div ref={wrapperRef} className={`${styles.wrapper} vizRoot`}>
      ...
      <svg
        ...
        role="img"
        aria-label="Average resale price by remaining lease years"
          ...
          {points.map(
            (point, index) =>
              labelIndices.has(index) && (
                <text
                  key={point.remainingLeaseYears}
                  x={xFor(index)}
                  y={plotHeight + 20}
                  className={styles.axisLabel}
                  textAnchor="middle"
                >
                  {point.remainingLeaseYears}
                </text>
              ),
          )}
      ...
      {hoveredPoint && hoverPos && (
        <div className={styles.tooltip} role="status" style={tooltipPosition(hoverPos, wrapperRef.current)}>
          <div className={styles.tooltipHeader}>{hoveredPoint.remainingLeaseYears} years remaining</div>
